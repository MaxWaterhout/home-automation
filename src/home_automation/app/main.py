...
def main():
    ...
    try:
        ...
        utils.print_log("Successfully logged in to Tado. Starting home monitoring loop...", last_message_container)
        ...
        utils.print_log("Process interrupted by user. Exiting gracefully with home status.", last_message_container)
        if tado_instance:
            try:
                tado_instance.set_home()
                utils.print_log("Successfully set HOME mode before exiting.", last_message_container)
